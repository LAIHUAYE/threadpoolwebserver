...
const char* doc_root = "/home/lhy/桌面/123/open/-/src";
...
http_conn::HTTP_CODE http_conn::process_read()//完整的HTTP解析
{
    ...
    while ( ( ( m_check_state == CHECK_STATE_CONTENT ) && ( line_status == LINE_OK  ) )
        ...
        switch ( m_check_state )//HTTP解析状态跳转
        {
            ...
            case CHECK_STATE_CONTENT://HTTP解析状态仍为正在解析.
            {
...
void save_to_file(char* body) {
    bool isHtml = false;
    int length=strlen(body);
    if (length >= 5 && strncmp(body, "<html", 5) == 0) {
        isHtml = true;
    }
    if(length>=14&&strncmp(body,"<!DOCTYPE html",14)==0){
        isHtml=true;
    }
    ...
    if (isHtml) {
     ...
     if (file.is_open()) {
        bool isUnixEncoding = true;
        for (int i = 0; i < length; i++) {
            if (body[i] == '\r') {
                isUnixEncoding = false;
                break;
            }
        }
        if(!isUnixEncoding){
        //转unix编码
        std::string content(body, length);
        std::string unix_content;
        for (int i = 0; i < content.length(); i++) {
            if (content[i] != '\r') {
                unix_content += content[i];
            }
        }
